...
<%
	...
	List<AtchFileVO> fileList = 
			(List<AtchFileVO>) request.getAttribute("fileList");
...
<body>
	<form action="update.do" method="post" enctype="multipart/form-data">
		...
		<table>
			...
			%>
						<div>
							<a href="<%= request.getContextPath() %>/download.do?fileId=<%= fileVO.getAtchFileId() %>&fileSn=<%= fileVO.getFileSn() %>"><%= fileVO.getOrignlFileNm() %></a>
						</div>
			<%
					}
				}
